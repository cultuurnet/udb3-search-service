...
byte — note that `php:8.2-fpm` is a moving base tag, so a fresh local build can
...
it: `php:8.2-fpm` is a mutable tag and the `apt`/`pecl` installs are unpinned, so a
